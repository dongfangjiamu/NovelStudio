...
function setConversationFeedback(text = "", kind = "error") {
  ...
  if (!text) {
    ...
    el.conversationFeedback.classList.remove("feedback-error", "feedback-success", "feedback-warn");
    return;
  }
  ...
  el.conversationFeedback.classList.remove("feedback-error", "feedback-success", "feedback-warn");
  if (kind === "success") {
    el.conversationFeedback.classList.add("feedback-success");
  } else if (kind === "warn") {
    el.conversationFeedback.classList.add("feedback-warn");
  } else {
    el.conversationFeedback.classList.add("feedback-error");
  }
}
...
function conversationPostAdoptionHint(thread) {
  const interview = interviewState(thread);
  const stage = interview?.stage_confirmation || null;
  const recommendation = stage?.action_recommendation || null;
  if (recommendation?.mode === "apply_and_split") {
    return "下一步建议：直接点“确认并拆出第一批结论”，系统会把这版阶段摘要写回项目设定，并把人物/卷纲/长期规则一起准备好。";
  }
  if (recommendation?.mode === "apply_summary") {
    return `下一步建议：点“${recommendation.primary_label || stageSummaryApplyLabel(thread?.scope)}”，先确认这版阶段摘要。`;
  }
  if (stage?.next_steps?.length) {
    return `下一步建议：${stage.next_steps
      .slice(0, 2)
...
function reportConversationSuccess(message, thread = selectedThread()) {
  const hint = conversationPostAdoptionHint(thread);
  const fullMessage = hint ? `${message} ${hint}` : message;
  setConversationFeedback(fullMessage, "success");
...
async function adoptConversationMessage(messageId, decisionType) {
  try {
    ...
    reportConversationSuccess(`已采纳为${conversationDecisionLabel(decisionType)}`);
  } catch (error) {
    reportConversationError(error);
  }
}
...
async function createConversationDecisionFromDraft({ threadId, decisionType, content, sourceLabel }) {
  ...
  reportConversationSuccess(`已从草案采纳为${conversationDecisionLabel(decisionType)}`);
}
